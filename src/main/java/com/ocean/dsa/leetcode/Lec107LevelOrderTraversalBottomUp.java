...
public class Lec107LevelOrderTraversalBottomUp {
    ...
    public List<List<Integer>> levelOrderBottom(TreeNode root) {
        ...
        while (!queue.isEmpty()) {
            ...
            result.addFirst(level); // Add to front instead of reversing later
        }
        return result;
    }

    public static void main(String[] args) {
        Lec107LevelOrderTraversalBottomUp obj = new Lec107LevelOrderTraversalBottomUp();

        // ----------- TEST CASES -----------

        // Case 1: normal balanced tree
        TreeNode t1 = TreeNodeFactory.buildTree(new Integer[]{3, 9, 20, null, null, 15, 7});
        System.out.println("Test 1 (expected: [[3],[9,20],[15,7]]): " + obj.levelOrderBottom(t1));

        // Case 2: single node
        TreeNode t2 = TreeNodeFactory.buildTree(new Integer[]{1});
        System.out.println("Test 2 (expected: [[1]]): " + obj.levelOrderBottom(t2));

        // Case 3: empty tree
        System.out.println("Test 3 (expected: []): " + obj.levelOrderBottom(null));

        // Case 4: skewed left tree
        TreeNode t4 = TreeNodeFactory.buildTree(new Integer[]{1, 2, null, 3, null, 4});
        System.out.println("Test 4 (expected: [[1],[2],[3],[4]]): " + obj.levelOrderBottom(t4));

        // Case 5: skewed right tree
        TreeNode t5 = TreeNodeFactory.buildTree(new Integer[]{1, null, 2, null, 3, null, 4});
        System.out.println("Test 5 (expected: [[1],[2],[3],[4]]): " + obj.levelOrderBottom(t5));

        // Case 6: complete binary tree
        TreeNode t6 = TreeNodeFactory.buildTree(new Integer[]{1, 2, 3, 4, 5, 6, 7});
        System.out.println("Test 6 (expected: [[1],[2,3],[4,5,6,7]]): " + obj.levelOrderBottom(t6));

        // Case 7: tree with nulls in between
        TreeNode t7 = TreeNodeFactory.buildTree(new Integer[]{1, 2, 3, null, 5, 6, null});
        System.out.println("Test 7 (expected: [[1],[2,3],[5,6]]): " + obj.levelOrderBottom(t7));
    }
